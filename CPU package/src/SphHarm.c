...
void SpherHarmonicArray(int degree,int order,int N,double *theta,double *phi,complex double *out) {
	...
	if ((costheta = malloc(sizeof(double)*N)) == NULL) {
		printf("Error allocating memory!\n");
		exit(1);
	}
	if ((Ymn = malloc(sizeof(double)*N)) == NULL) {
		printf("Error allocating memory!\n");
		exit(1);
	}
...
double AssocLegendre(double x,int degree, int order) {
	...
	if (degree == 0) {
		...
		new = fact*pow(-1.0,(double)tmporder);
		new = new*pow(1.0-pow(x,2.0),((double)tmporder/2.0));
	}
	
	l = tmporder + 1;
	for (i = 2;i < len;i++) {		
		tmp = x * (2.0 * (double)l - 1.0) * new/((double)(l-tmporder)) - (double)(l+tmporder-1)/((double)(l-tmporder))*old;
		old = new;
		new = tmp;
		l++;
	}
...
void AssocLegendreArray(int degree, int order,int N,double *in,double *out) {
	...
	if ((old = calloc(N,sizeof(double))) == NULL) {
		printf("Error allocating memory!\n");
		exit(1);
	}
	if ((tmp = malloc(sizeof(double)*N)) == NULL) {
		printf("Error allocating memory!\n");
		exit(1);
	}
