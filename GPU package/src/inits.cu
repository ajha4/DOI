...
//	phan->mus = iniparser_getdouble(ini,"Phantom:mus",-1) + 0*I;
	...
	phan->mu_abs = (complex_double *) malloc (sizeof(complex_double)*phan->no_tiss);
	phan->mu_sc = (complex_double *) malloc (sizeof(complex_double)*phan->no_tiss);
...
//	tiss_sc_str = (char *)(malloc (sizeof(char)*16));
	...
	for (i=1; i < phan->no_tiss; i++){
		...
		phan->mu_abs[i] = iniparser_getdouble(ini, tiss_abs_str, 0.01) + 0.0*I;
		phan->mu_sc[i] = iniparser_getdouble(ini, tiss_sc_str, 1.0) + 0.0*I;
	}

	phan->mu_abs[0] = 0.0 + 0.0*I;
    phan->mu_sc[0] = 0.0 + 0.0*I; 
        ...
        for (j=geom->bounY; j<geom->nY + geom->bounY; j++){
           for (k=geom->bounX; k<geom->nX+geom->bounX; k++){
				...
				if(flag == 1){
					...
					if(phan->tiss_type[r_ind] == 2)
						printf("Tissue type 2 read for r_ind as (%d %d %d) voxel with r_ind = %d \n", i,j,k,r_ind); 
				}
				else
