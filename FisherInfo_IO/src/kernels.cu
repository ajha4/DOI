...
//Changed the value of cm_tmp to info_stat.cm
...
__global__ void prop_scat_dev (doublecomplex *out_dist, Info_Dyn info_dyn, short layer_start, SHORT block_dep, SHORT flag)
{
  ...
  int m;
  cnt = 0;
  for(l=0; l<(nL_dev+1); l++){
     for(m=-l; m<=l;m++){      
//  for (cnt = 0; cnt < (nL_dev+1)*(nL_dev+1); cnt++){
         sp_ind = get_voxind_spind(block_dep, block_row, block_col,i,j,k,cnt);
         if(flag) {
//			 l = (int) sqrtf(cnt); 
    	     out_dist[sp_ind] = cm_tmp*pow (__int2double_rn(info_stat.g),__int2double_rn(l))   * out_dist[sp_ind] * info_stat.mu_sc[info_dyn.tiss_type[r_ind]] ;
		 }
         else{
		    out_dist[sp_ind] = out_dist[sp_ind] * info_stat.mu_sc[info_dyn.tiss_type[r_ind]] ;
         }
       cnt++;
       }	
     }
...
//        printf("Invoking compute_propabs with cnt = %d  \n", cnt);
        ...
        if(parameters.flag_scat){
             ...
             for(block_dep = 0; block_dep < parameters.num_layers; block_dep = block_dep + BLK_SIZE_Z){
                prop_scat_dev<<<dim_grid, dim_block>>>(out_dev, info_dyn_dev,parameters.layer_start,block_dep,parameters.flag);
