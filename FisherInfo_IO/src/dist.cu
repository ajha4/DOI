				...
				for ( m = -l; m <= l; m++){
					if ( m == 0){ 
                        ...
                        for(ip = geom->bounZ; ip < i; ip++){
							...
							tmp = tmp* (exp(-(phan->mu_sc[tiss_type] + phan->mu_abs[tiss_type]) * geom->delZ));
                            //tmp = tmp * ((-(phan->mu_sc[r_indp] +phan->mu_abs[r_indp])  * geom->delZ ).cexp());
                        }
						tiss_type = phan->tiss_type[r_ind];
					//	printf("tmp is %e for (%d %d %d) voxel\n", tmp.real(),i,j,k);
						src_dist[VOX_TO_SPIND(r_ind,cnt,(nL+1)*(nL+1))] = pow(phan->g,l) * phan->mu_sc[tiss_type] / (phan->mu_sc[tiss_type] + phan->mu_abs[tiss_type]) * sqrt(((2.0*(double)l)+1.0)/(4.0*M_PI))  * tmp * (1 - exp(-(phan->mu_sc[tiss_type]+phan->mu_abs[tiss_type])*geom->delZ)) / (geom->delX * geom->delY * geom->delZ);
					//printf("%e + %e is the source dist generated after the KXE term in the (%d %d %d) voxel whose linear ind is %d for cnt = %d \n",  src_dist[VOX_TO_SPIND(r_ind,cnt,(nL+1)*(nL+1))].real(), (src_dist[VOX_TO_SPIND(r_ind,cnt,(nL+1)*(nL+1))] ).imag(), i-geom->bounZ,j-geom->bounY,k-geom->bounX, VOX_TO_SPIND(r_ind,cnt,(nL+1)*(nL+1)),cnt);
					}
				cnt++;
				}
			}
...
//			if ( sqrt(pow(beam_src->pos_x - geom->xs[k],2)  + pow(beam_src->pos_y - geom->ys[j],2)) <= beam_src->diam/2.0){
			if( k == geom->bounX + geom->nX/2.0 && j == geom->bounY + geom->nY/2.0){
				...
				for(ip=geom->bounZ ; ip < geom->nZ + geom->bounZ; ip++){
	  				 r_indp = ip* (geom->nX + 2*geom->bounX )* (geom->nY + 2*geom->bounY) + j* (geom->nX + 2*geom->bounX) + k;
				     tmp = tmp * exp(-(phan->mu_sc[phan->tiss_type[r_indp]] +phan->mu_abs[phan->tiss_type[r_indp]]) * geom->delZ);
				}

//				printf("%e is the value of the 0 term in the transmitted image for voxel (%d %d %d) with r_ind as %d \n", tmp.real(),i,j,k,r_ind);
...
/*                if(out_dist[VOX_TO_SPIND(r_ind,(l*l+l),geom->no_vox)].real() > 0.0){
                ...
                for (sp = 0;sp<num_xs;sp++) {
                    val = val+ 2.0*M_PI*C*geom->delX * geom->delY / sqrt(4.0*M_PI) * sqrt(2.0*(flt_doub)l +1.0) *
...
//            if(j == geom->nY/2 && k == geom->nX/2)
  //             printf("%e is trans_image for center voxel %d \n", trans_image[(j-geom->bounY)*geom->nX + (k-geom->bounX)], (j-geom->bounY)*geom->nX + (k-geom->bounX));
		}
	}
	free(out);
	free(xs);
    ...
    for (j=geom->bounY;j<geom->nY + geom->bounY;j++) {
       for (k=geom->bounX;k<geom->nX + geom->bounX;k++) {
			...
			if ( sqrt(pow(beam_src->pos_x - geom->xs[k],2)  + pow(beam_src->pos_y - geom->ys[j],2)) < beam_src->diam/2.0){
				...
				//printf("%e is the value of the 0 term phase image for voxel (%d %d %d) with r_ind as %d \n", tmp.real(),i,j,k,r_ind);
                val = val+ tmp;   // mu_int(cor_src,cor_dest,subvox_src, subvox_dest);
			}
			}
			for (l=0;l<= nL;l++) {
...
complex_double * generate_ref_image(complex_double *out_dist){  
    ...
    for (j=geom->bounY;j<geom->nY + geom->bounY;j++) {
       for (k=geom->bounX;k<geom->nX + geom->bounX;k++) {
			...
			ref_image[(j-geom->bounY)*geom->nX + (k-geom->bounX)] = val*beam_src->mag;
		}
	}
	free(out);
	free(xs);
...
complex_double * downsample_image(complex_double *img, int ds_factor){
    complex_double *img_ds;
    img_ds = (complex_double *) malloc(sizeof(complex_double)*geom->nX*geom->nY/(ds_factor*ds_factor));
    int i,j;
    int nX, nY;
    nX = geom->nX/ds_factor;
    nY = geom->nY/ds_factor;
    int ds_indx, ds_indy;
    for(i = 0; i < nX; i++){
       for(j=0; j< nY; j++){
         for(ds_indx=0; ds_indx < ds_factor; ds_indx++){
            for(ds_indy=0; ds_indy < ds_factor; ds_indy++){
               img_ds[i*nY + j] = img_ds[i*nY + j] + img[(i*ds_factor+ds_indx)*geom->nY + j*ds_factor + ds_indy]; 
            }
         }
      }
    }
...
//	#pragma omp parallel for 
 ...
 if (face_x <= face && face_x >= -face &&  face_y <= face && face_y >= -face  && face_z <= face && face_z >= -face && sph_y[omega_count] * face_y >= 0){
                            ...
                            fact_self_vox[r_ind_self][omega_count] = ( 1 - exp( -(phan->mu_abs[tiss_num] + phan->mu_sc[tiss_num]) * dist_self)) * sin(theta_self[omega_count]);
                            }
                             r_ind_self++;
                            }
                        }
...
/*
 ...
 if (face_x <= face && face_x >= -face &&  face_y <= face && face_y >= -face  && face_z <= face && face_z >= -face && sph_y[omega_count] * face_y >= 0){
						    ...
						    dist_self = sqrt(pow(ii_self*2.0/geom->self_sub_vox - cube_x,2.0) + pow(jj_self*2.0/geom->self_sub_vox - cube_y,2.0) + pow(kk_self*2.0/geom->self_sub_vox - cube_z,2.0)) * geom->delX/2.0;
                            fact_self_vox =  ( 1 - exp( -(phan->mu_abs[tiss_num] + phan->mu_sc[tiss_num]) * dist_self))  *  sin(theta_self[omega_count]) ;
...
//          if (rad <= beam_src->diam/2.0) {
            if (k == geom->nX/2 + geom->bounX && j == geom->nY/2 + geom->bounY ) {
                ...
                for(ip=geom->bounZ ; ip < geom->nZ + geom->bounZ; ip++){
                     r_indp = ip* (geom->nX + 2*geom->bounX )* (geom->nY + 2*geom->bounY) + j* (geom->nX + 2*geom->bounX) + k;
                     tmp_var = tmp_var * exp(-(phan->mu_sc[phan->tiss_type[r_indp]] +phan->mu_abs[phan->tiss_type[r_indp]]) * geom->delZ);
                }

                //printf("%e is the value of the 0 term computephoton for voxel (%d %d %d) with r_ind as %d \n", tmp_var.real(),i,j,k,r_ind);
                val = val+ tmp_var;  	
            }
    ...
    ret_tmp = ret;
	printf("Contribution from bottom face outgoing is %e \n", ret_tmp.real());
  ...
  // Side Face 1 and 2: outgoing and incoming respectively
...
//	printf("Contribution from side face 1 outgoing is %e \n", ret - ret_tmp );
	ret_tmp = ret;

    k = 2*geom->bounX + geom->nX;
    ...
    // Side Face 2 and 1: outgoing and incoming
...
//	printf("Contribution from side face 2 is %e \n", ret - ret_tmp );
    ...
    // Side Face 3 and 4: outgoing and incoming
    deltheta = (M_PI)/(double)(num);
    delphi = (M_PI)/(double)(num);
    for (i=0;i<num;i++) {
        for (j=0;j<num;j++) {
...
//	printf("Contribution from side face 3 is %e \n", ret - ret_tmp );
   ...
   // Side Face 4 and 3: outgoing and incoming:
    deltheta = (M_PI)/(double)(num);
    delphi = (M_PI)/(double)(num);
    for (i=0;i<num;i++) {
        for (j=0;j<num;j++) {
...
int StopIteration(double *fi, int n, complex_double *W, complex_double *out){
    int i,j,k,l,m;
    int cnt;
    double sumOut,sumW;
    double hi;
    ...
    for (j=0;j<geom->nY;j++){
       for (k=0;k<geom->nX;k++){
			 r_ind = i* (geom->nX + 2*geom->bounX )* (geom->nY + 2*geom->bounY) + j* (geom->nX + 2*geom->bounX) + k;
			for (cnt=0;cnt< (nL+1)*(nL+1) ;cnt ++) {
                  sumW += W[VOX_TO_SPIND(r_ind,cnt,(nL+1)*(nL+1))].real();
                  sumOut += out[VOX_TO_SPIND(r_ind,cnt,(nL+1)*(nL+1))].real();
            }
        }
     }
