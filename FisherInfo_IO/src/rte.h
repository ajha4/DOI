...
typedef struct {
	...
	SHORT no_tiss;
	flt_doub *mu_abs, *mu_sc;
    doublecomplex mut_mean;
} Phantom;
...
typedef struct {
	...
	flt_doub mu_tot[MAX_TISS_NUM];
	flt_doub mu_sc[MAX_TISS_NUM];
...
typedef struct {
    ...
    SHORT flag_scat; // The flag that denotes whether the scatter operation will occur along with the absorption operation. 
} THREAD_PARAMETERS;
...
void Neumann(complex_double* src_host, complex_double *out_host, int flag);
