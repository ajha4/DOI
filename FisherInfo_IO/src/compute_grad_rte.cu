...
int main(int argc, char** argv )
{
    ...
    double *g;
    double *grada,*grads,*grada2,*grads2;
    complex_double delg, delg2;
    ...
    grada2 = (double *)malloc(sizeof(double)*phan->no_tiss);
    grads2 = (double *)malloc(sizeof(double)* phan->no_tiss);
    ...
    resFID = fopen("Residual.out","w");
        for (itt = 0;itt < numIT;itt++ ) {
			...
			flag_net = 1;
            phan->mu_sc[1] = 0.8 + round(itt/2.0)*0.05;
            phan->mu_abs[1] = 0.008 + floor(itt/2.0)*0.0005;
            ...
            memset(out2, 0, sizeof(complex_double)*geom->no_vox  * pow(nL+1,2));
            memset(W, 0, sizeof(complex_double)*geom->no_vox  * pow(nL+1,2));
...
//          OutputImage(geom,gbar,"Hi.dat");
            ...
            for (tiss_idx = 1; tiss_idx < phan->no_tiss; tiss_idx++) {
                memset(out3, 0, sizeof(complex_double)*geom->no_vox  * pow(nL+1,2));
				...
				if(flag_grada[tiss_idx] == 0){
                   ...
                   memset(src, 0, sizeof(complex_double)*geom->no_vox  * pow(nL+1,2));
	                ...
	                for (j = 0;j<geom->nX*geom->nY;j++) {
                      if(g[j] ||  gbar[j].real()){
        	            grada[tiss_idx] = grada[tiss_idx] - 2.0*av_mua*dgbar[j].real()/(g[j]-gbar[j].real());
        	            grada2[tiss_idx] = grada2[tiss_idx] - 2.0*av_mua*(g[j]-gbar[j].real())*dgbar[j].real();
...
*/
            ...
            for (tiss_idx = 1; tiss_idx < phan->no_tiss ; tiss_idx++) {
               ...
               memset(out3, 0, sizeof(complex_double)*geom->no_vox  * pow(nL+1,2));
               memset(tmp1, 0, sizeof(complex_double)*geom->no_vox  * pow(nL+1,2));
			   ...
			   if(flag_grads[tiss_idx] == 0){
            	    ...
            	    for (j = 0;j<geom->nX*geom->nY;j++) {
                        if(g[j] || gbar[j].real()){
						grads[tiss_idx] = grads[tiss_idx] - 2.0*av_mus*dgbar[j].real()/(g[j]-gbar[j].real());
						grads2[tiss_idx] = grads2[tiss_idx] - 2.0*av_mus*((g[j]-gbar[j].real()))*dgbar[j].real();
                	  } 
                    }
...
/*
				...
				if ( flag_grada[tiss_idx] == 0){
	        	   phan->mu_abs[tiss_idx] = phan->mu_abs[tiss_idx] - av_mua*stepsizea*grada[tiss_idx];
				   if(phan->mu_abs[tiss_idx] < 0)
						phan->mu_abs[tiss_idx] = phan->mu_abs[tiss_idx] + 2*av_mua*stepsizea*grada[tiss_idx];
				}
				if ( flag_grads[tiss_idx] == 0){
...
*/				
				printf("Value of mua = %e mus = %e \n", phan->mu_abs[tiss_idx], phan->mu_sc[tiss_idx]);
				printf("Value of mua grad1 = %e musgrad1 = %e \n", grada[tiss_idx], grads[tiss_idx]);
				printf("Value of mua grad2 = %e musgrad2 = %e \n", grada2[tiss_idx], grads2[tiss_idx]);
//				}
            ...
            for (i=0;i<geom->nX*geom->nY;i++) {
                if(gbar[i].real()-g[i] != 0.0 ){
                   delg = delg + log((gbar[i].real()-g[i])*(gbar[i].real()-g[i]));
                }
